...
export const confirm = async (email) => {
  ...
  const getSubscriberByEmailUrl = `${API_URL}/subscribers/${decodeURIComponent(
    email,
  )}`;
  ...
  if (!subscriber.email) {
    ...
    return {
      statusCode: 200,
      success: false,
      message: `It doesn't look like anyone's signed up with this email address: ${decodeURIComponent(
        email,
      )}.`,
    };
  }
  ...
  const deleteFromGroupUrl = `${API_URL}/groups/${process.env.NEXT_PUBLIC_MAILERLITE_GROUP_ID_NEEDS_TO_CONFIRM}/subscribers/${subscriber.id}`;
  ...
  const addToGroupUrl = `${API_URL}/groups/${process.env.NEXT_PUBLIC_MAILERLITE_GROUP_ID_NEWSLETTER}/subscribers`;
