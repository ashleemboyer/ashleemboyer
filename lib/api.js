...
export function getAllPosts() {
  ...
  yearDirectoryNames.forEach((yearDirectoryName) => {
    ...
    monthDirectoryNames.forEach((monthDirectoryName) => {
      ...
      fileNames.forEach((fileName) => {
        ...
        const parsedBodyPieces = bodyPieces.map((piece, index) => {
        ...
        const parsedForSite = marked(parsedBodyPieces.join(''));
        const parsedForFeed = marked(body).replace(/<br>/gm, '<br />');
        const withoutTags = parsedForSite.replace(/<[^>]*>/g, '');
        ...
        sitePosts.push({
          meta,
          content: parsedForSite,
          fileName: slug,
        });

        feedPosts.push({
          meta,
          content: parsedForFeed,
          fileName: slug,
        });
      });
    });
  ...
  return { forSite: sitePosts, forRSSFeed: feedPosts };
}
...
export function getPostBySlug(slug) {
  const { forSite } = getAllPosts();

  for (let i = 0; i < forSite.length; i++) {
    const post = forSite[i];
